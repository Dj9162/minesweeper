...
def run_games(num_games=30, seed_base=1):
    width, height, mines = 9, 9, 10
    results = []

    print(f"\nRunning {num_games} BEGINNER games")
    ...
    for i in range(num_games):
        ...
        print(f"[{i}] Win: {score['win']}, Terminated: {score['terminated']}, Steps: {score['steps']}")
        results.append({
            "win": score["win"],
            "steps": score["steps"],
...
def main():
    ...
    for k, v in summary.items():
        print(f"  {k}: {v}")
    with open("results_beginner.json", "w") as f:
